<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    <x-nav-link :href="route('app.launcher')" :active="request()->routeIs('app.launcher')">
                        {{ __('Apps') }}
                    </x-nav-link>
                    ...
                    @if(auth()->user()?->hasPermission('platform.tenant_admin.access'))
                        <x-nav-link :href="route('tenant-admin.users.index')" :active="request()->routeIs('tenant-admin.*')">
                            {{ __('Tenant Admin') }}
                        </x-nav-link>
                    @endif
                </div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link :href="route('app.launcher')" :active="request()->routeIs('app.launcher')">
                {{ __('Apps') }}
